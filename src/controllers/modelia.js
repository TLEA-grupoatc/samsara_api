module.exports = app => {
  ...
  app.preguntarPonderacionOp = async (req, res) => {
    ...
    CONTEXTO DE BASE DE DATOS:
    Asume que ya existe un modelo de Sequelize definido así (ya importado):
    const podeope = app.database.models.PonderacionOperador;
    ...
    - operador: STRING
    - ponderacion: INTEGER
    - examen_maxipista: INTEGER
    - examen_maxipista_calificacion: INTEGER
    - auditoria_maxipista: INTEGER
    - auditoria_maxipista_calificacion: INTEGER
    - exceso_velocidad: INTEGER
    - exceso_velocidad_calificacion: INTEGER
    - paradano_autorizada: INTEGER
    - paradano_autorizada_calificacion: INTEGER
    - usode_movil: INTEGER
    - usode_movil_calificacion: INTEGER
    - distanciade_seguimiento: INTEGER
    - distanciade_seguimiento_calificacion: INTEGER
    - somnoliento: INTEGER
    - somnoliento_calificacion: INTEGER
    - fecha_creacion: STRING // 'YYYY-MM-DD HH:mm:ss'

    VARIABLES DISPONIBLES:
    - 'podeope': El modelo de Sequelize.
    - 'sequelize': La instancia de Sequelize (para 'fn' y 'col').
    - 'Op': El objeto 'Op' de Sequelize.
    ...
    REGLAS DE SALIDA:
    - Genera ÚNICAMENTE el código de la consulta.
    - NO incluyas 'await', variables, comentarios ni explicaciones.
    - NO uses bloques de código \`\`\`.
    - La salida debe ser una expresión de Sequelize lista para \`return \${codigo};\`.
