...
try {
    ...
    const systemPrompt = `
      ...
      3. 'pagadas' -> significa { estado: 'COMPLETAS' }
      ...
      Salida: Liquidaciones.findAll({ where: { fecha: { [Op.like]: '%2025-10-1%' }, unidad: 5 } })

      Pregunta: ¿Cuantos operadores se han liquidado esta semana de la unidad 5?
      Salida: Liquidaciones.findAll({ where: { fecha: { [Op.like]: '%2025-10-%' }, unidad: 5 } })

      Pregunta: ¿Cuantos operadores tienen liquidación pendiente por firmar de la unidad 5?
      Salida: Liquidaciones.findAll({ where: { estado: 'PENDIENTE DE FIRMA', unidad: 5 } })

      Pregunta: ¿Cuales son los motivos de rechazo en liquidacion mas frecuentes de la unidad 5?
      Salida: Liquidaciones.findAll({ where: { estado: 'RECHAZADO', unidad: 5 } })
