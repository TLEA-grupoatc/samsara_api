...
module.exports = app => {
    ...
    app.reporteInmovilizadores = async (req, res) => {
        ...
        const getAllImmobilizerData = async () => {
            for (const lt of listatractos) {
                try {
                    ...
                    console.error(`Error con tracto ${lt}:`, error);
                }

                await sleep(1000); // pausa de 1 segundo entre cada llamada
            }
            ...
            const latestStates = Object.values(agrupadoPorVehiculo);

            res.json({
                OK: true,
                reporte: allImmobilizerData,
                reporteresumdo: latestStates
            });
        };
    ...
    app.obtenerReporteUltimaLocacion = async (req, res) => {
        var fechaHoy = moment(new Date()).format('YYYY-MM-DD');
